...
in
{
        ...
        lib.types.submodule (
          { config, name, ... }:
          {
            options = {
              ...
              insecureNoPassword = lib.mkOption {
                type = lib.types.bool;
                default = false;
                description = ''
                  Use an empty password for the repository.
                '';
                example = true;
              };
  ...
  config = lib.mkIf cfg.enable {
    ...
    ++ lib.mapAttrsToList (n: v: {
      assertion = lib.xor (v.insecureNoPassword == false) (v.passwordFile == null);
      message = "services.restic.backups.${n}: exactly one of passwordFile or insecureNoPassword should be set";
    }) cfg.backups;
    ...
    systemd.user.services = lib.mapAttrs' (
      ...
      let
        ...
        ++ lib.optional backup.insecureNoPassword "--insecure-no-password";
